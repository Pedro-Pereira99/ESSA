...
int main(void)
{
  ...
  int32_t gyro_axes[3]; //Array to save the data of angular velocity measured in each axis of space (X Y and Z);
  ...
  while (1)
  {
	 ...
	 if(tim2Flag == 1){
		  ...
		  status_gyro = IKS01A3_MOTION_SENSOR_GetAxes(IKS01A3_LSM6DSO_0, MOTION_GYRO, gyro_axes); // This function will save the data in the array gyro_axes, and return a value that can be useful to finding errors;
		  status_acc = IKS01A3_MOTION_SENSOR_GetAxes(IKS01A3_LIS2DW12_0, MOTION_ACCELERO, acc_axes); // This function will save the data in the array acc_axes, and return a value that can be useful to finding errors;

		  if(status_acc == BSP_ERROR_NONE){ // If no error is detected when reading the data
			  // This should work if the data are integers. Furthermore, additional text such as 'acceleration on X axis: ' was avoided because
			  // The data printed on the serial monitor will be used in MATLAB, so this code tries to print in a way that makes the "copying and pasting" easier

			  HAL_UART_Transmit_IT(&huart2, buffer, sprintf(buffer, "%d ", acc_axes[0])); // Transmits acceleration in the X axis
			  ...
			  HAL_UART_Transmit_IT(&huart2, (uint8_t *)welcome_message, sizeof(char)*strlen(welcome_message)); // Transmit the error message;
		  }

		  if(status_acc == BSP_ERROR_NONE){ // If no error is detected when reading the data

			  HAL_UART_Transmit_IT(&huart2, buffer, sprintf(buffer, "%d ", gyro_axes[0])); // Transmits angular velocity in the X axis
			  HAL_UART_Transmit_IT(&huart2, buffer, sprintf(buffer, "%d ", gyro_axes[1])); // Transmits angular velocity in the Y axis
			  HAL_UART_Transmit_IT(&huart2, buffer, sprintf(buffer, "%d \r\n", gyro_axes[2])); // Transmits angular velocity in the Z axis
		  }
		  else{
			  HAL_UART_Transmit_IT(&huart2, (uint8_t *)welcome_message, sizeof(char)*strlen(welcome_message)); // Transmit the error message;
		  }
